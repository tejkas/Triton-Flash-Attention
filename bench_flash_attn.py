...
def bench_one(Z, H, N, D, dtype, provider):
    ...
    if provider == "triton":
        fn = lambda: flash_attn_forward(q, k, v, sm_scale)
    elif provider == "torch":
        fn = lambda: F.scaled_dot_product_attention(q, k, v, is_causal=False, scale=sm_scale)
    else:
...
def main():
    ...
    for N in (512, 1024, 2048, 4096, 8192, 16384):
        try:
            t_ms, t_tf = bench_one(Z, H, N, D, dtype, "triton")
            r_ms, r_tf = bench_one(Z, H, N, D, dtype, "torch")
